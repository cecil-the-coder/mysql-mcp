...
const MYSQL_RESERVED_KEYWORDS: &[&str] = &[
    ...
    "OUT", "OUTER", "OUTFILE", "OVER", "PARTITION", "PERCENT_RANK", "PRECISION",
    "PRIMARY", "PROCEDURE", "PURGE", "RANGE", "RANK", "READ", "READS", "READ_WRITE",
    "REAL", "RECURSIVE", "REFERENCES", "REGEXP", "RELEASE", "RENAME", "REPEAT",
...
/// Uses case-insensitive comparison without allocating a temporary string.
fn is_mysql_reserved_keyword(name: &str) -> bool {
    MYSQL_RESERVED_KEYWORDS.iter().any(|&kw| kw.eq_ignore_ascii_case(name))
}
    ...
    for (key, val) in std::env::vars() {
            ...
            .and_then(|s| s.strip_suffix(SUFFIX))
        {
            ...
            if is_mysql_reserved_keyword(&schema_name) {
                eprintln!(
                    "Error: schema name '{schema_name}' in {key} is a MySQL reserved keyword; \
                     reserved keywords cannot be used as unquoted schema names — skipping"
                );
                continue;
            }
