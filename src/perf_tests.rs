...
mod perf_tests {
    ...
    async fn perf_sequential_select_latency() {
        ...
        // connection handler (server connect_timeout=10s).
        {
            ...
            for _ in 0..WARMUP {
                crate::query::read::execute_read_query(pool, "SELECT 1", &crate::sql_parser::StatementType::Select, false, 0).await.unwrap();
            }
        }
    ...
    async fn perf_sequential_join_query() {
        ...
        for _ in 0..N {
            let t = Instant::now();
            crate::query::read::execute_read_query(pool, sql, &crate::sql_parser::StatementType::Select, false, 0).await.unwrap();
            samples.push(t.elapsed().as_secs_f64() * 1000.0);
        }
        let stats = compute(samples, wall.elapsed().as_secs_f64() * 1000.0);
    ...
    async fn perf_read_isolation_comparison() {
        ...
        for _ in 0..N {
            let t = Instant::now();
            crate::query::read::execute_read_query(pool, "SELECT 1", &crate::sql_parser::StatementType::Select, true, 0).await.unwrap();
