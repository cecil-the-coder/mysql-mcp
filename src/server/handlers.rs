...
impl SessionStore {
    ...
    pub(crate) async fn handle_server_info(
        ...
        match rows {
            Ok(rows) if !rows.is_empty() => {
                ...
                let read_only_raw: String = row.try_get("read_only").unwrap_or_else(|e| {
                    tracing::debug!("Failed to get read_only: {}", e);
                    String::new()
                });
                let read_only = match read_only_raw.to_uppercase().as_str() {
                    "ON" => true,
                    "OFF" => false,
                    other => {
                        tracing::warn!("Unexpected @@read_only value: {:?}", other);
                        false
                    }
                };
    ...
    pub(crate) async fn handle_explain_plan(
        ...
        match crate::query::explain::run_explain(&ctx.pool, &sql, query_timeout_ms).await {
            Ok(plan) => {
