...
pub(crate) fn query_schema() -> Arc<serde_json::Map<String, serde_json::Value>> {
    Arc::new(rmcp::model::object(json!({
        ...
        "properties": {
            ...
            "session": {
                "type": "string",
                "description": "Named session to route this query to (omit for default connection). Use 'default' or a session name from connect."
            }
        },
        "required": ["sql"]
    })))
...
pub(crate) fn schema_info_schema() -> Arc<serde_json::Map<String, serde_json::Value>> {
    Arc::new(rmcp::model::object(json!({
        "type": "object",
        "properties": {
            ...
            "session": {
                "type": "string",
                "description": "Named session to use (omit for default connection). Use 'default' or a session name from connect."
            }
        },
        "required": ["table"]
    })))
...
pub(crate) fn server_info_schema() -> Arc<serde_json::Map<String, serde_json::Value>> {
    Arc::new(rmcp::model::object(json!({
        "type": "object",
        "properties": {
            "session": { "type": "string", "description": "Named session to use (omit for default connection). Use 'default' or a session name from connect." }
        },
        "required": []
    })))
}

pub(crate) fn connect_schema() -> Arc<serde_json::Map<String, serde_json::Value>> {
    Arc::new(rmcp::model::object(json!({
        "type": "object",
        "properties": {
            ...
            "host": { "type": "string", "description": "Database host (required unless using preset)" },
            "port": { "type": "integer", "description": "Database port (default: 3306).", "minimum": 1, "maximum": 65535 },
            "user": { "type": "string", "description": "Database username" },
            "password": { "type": "string", "description": "Database password (optional; use empty string for passwordless login)." },
...
pub(crate) fn disconnect_schema() -> Arc<serde_json::Map<String, serde_json::Value>> {
    Arc::new(rmcp::model::object(json!({
        "type": "object",
        "properties": {
            "name": { "type": "string", "description": "Session name to disconnect. Use the name that was provided to connect." }
        },
        "required": ["name"]
    })))
}

pub(crate) fn list_sessions_schema() -> Arc<serde_json::Map<String, serde_json::Value>> {
    Arc::new(rmcp::model::object(json!({
        "type": "object",
        "properties": {},
        "required": []
...
pub(crate) fn explain_plan_schema() -> Arc<serde_json::Map<String, serde_json::Value>> {
    Arc::new(rmcp::model::object(json!({
        "type": "object",
        "properties": {
            "sql": {
            ...
            "session": {
                "type": "string",
                "description": "Named session to use (default: 'default'). Use 'default' or a session name from connect."
            }
        },
        "required": ["sql"]
    })))
...
pub(crate) fn list_tables_schema() -> Arc<serde_json::Map<String, serde_json::Value>> {
    Arc::new(rmcp::model::object(json!({
        "type": "object",
        "properties": {
            "database": {
