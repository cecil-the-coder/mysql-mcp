...
async fn test_get_columns_for_table() {
    ...
    sqlx::query(
        "CREATE TABLE IF NOT EXISTS test_schema_cols (id INT PRIMARY KEY, name VARCHAR(100))",
    )
    .execute(pool)
    .await
    .unwrap();
...
async fn test_indexed_columns_cache_hit() {
    ...
    let first = introspector
        .list_indexed_columns("test_indexed_cache", db)
        .await
        .unwrap();
    ...
    assert!(
        ...
        "idx_val should be in indexed cols"
    );
    assert!(
        first.iter().any(|c| c.eq_ignore_ascii_case("id")),
        "PRIMARY should be in indexed cols"
    );

    sqlx::query("DROP TABLE IF EXISTS test_indexed_cache")
        .execute(pool.as_ref())
        .await
        .ok();
}
...
async fn test_indexed_columns_cache_invalidated_on_ddl() {
    ...
    sqlx::query("CREATE TABLE IF NOT EXISTS test_idx_inval (id INT PRIMARY KEY, val VARCHAR(50))")
        .execute(pool.as_ref())
        .await
        .unwrap();
    ...
    assert!(
        ...
        "val should be indexed after ALTER"
    );

    sqlx::query("DROP TABLE IF EXISTS test_idx_inval")
        .execute(pool.as_ref())
        .await
        .ok();
}
...
async fn test_list_composite_indexes() {
    ...
    let indexes = introspector
        .list_composite_indexes("test_composite_idx", db)
        .await
        .unwrap();
    ...
    sqlx::query("DROP TABLE IF EXISTS test_composite_idx")
        .execute(pool.as_ref())
        .await
        .ok();
}
...
async fn test_generate_suggestions_single_unindexed() {
    ...
    let suggestions = introspector
        .generate_index_suggestions("test_suggest_single", db, &["category".to_string()])
        .await;
    ...
    sqlx::query("DROP TABLE IF EXISTS test_suggest_single")
        .execute(pool.as_ref())
        .await
        .ok();
}
...
async fn test_generate_suggestions_composite_index() {
    ...
    sqlx::query("DROP TABLE IF EXISTS test_suggest_composite")
        .execute(pool.as_ref())
        .await
        .ok();
}
...
async fn test_generate_suggestions_low_cardinality() {
    ...
    let introspector = SchemaIntrospector::new(pool.clone(), 60);
    let suggestions = introspector
        .generate_index_suggestions("test_suggest_lowcard", db, &["active".to_string()])
        .await;
    ...
    sqlx::query("DROP TABLE IF EXISTS test_suggest_lowcard")
        .execute(pool.as_ref())
        .await
        .ok();
}
...
fn test_is_low_cardinality_type() {
    assert!(
        !is_low_cardinality_type("tinyint"),
        "bare tinyint is NOT low cardinality (256 values)"
    );
    assert!(is_low_cardinality_type("bool"), "bool is low cardinality");
    assert!(
        is_low_cardinality_type("boolean"),
        "boolean is low cardinality"
    );
    ...
    assert!(
        is_low_cardinality_type("tinyint(1)"),
        "tinyint(1) is low cardinality (boolean alias)"
    );
    assert!(
        is_low_cardinality_type("TINYINT(1)"),
        "TINYINT(1) case-insensitive boolean alias"
    );

    assert!(
        !is_low_cardinality_type("int"),
        "int is not low cardinality"
    );
    assert!(
        !is_low_cardinality_type("varchar"),
        "varchar is not low cardinality"
    );
    assert!(
        !is_low_cardinality_type("bigint"),
        "bigint is not low cardinality"
    );
    assert!(
    ...
    assert!(
        ...
        "bit(8) is NOT low cardinality (256 values)"
    );
    assert!(
...
async fn test_schema_cache_ttl_expiry_triggers_refetch() {
    ...
    // Use a unique table name to avoid races with other concurrent tests.
    let marker = format!("_ttl_expiry_test_{}", std::process::id());
    ...
    // 1-second TTL.
    let introspector = SchemaIntrospector::new(pool.clone(), 1);

    // Prime the cache (first fetch).
    let before = introspector.list_tables(db).await.unwrap();
    assert!(
        !before.iter().any(|t| t.name == marker),
        "marker should not exist yet"
    );

    // Create the marker table between the two list_tables calls.
    sqlx::query(&format!("CREATE TABLE `{marker}` (id INT)"))
        .execute(pool.as_ref())
        .await
        .unwrap();

    // Immediately after creation: the 1-second cache is still fresh,
    // so list_tables should NOT yet see the new table.
    let cached = introspector.list_tables(db).await.unwrap();
    assert!(
    ...
    // Now the cache is stale: list_tables must re-fetch and see the new table.
    let after = introspector.list_tables(db).await.unwrap();
